...
use App\Models\admin\AdsPackages_m;
use App\Models\admin\Login_m;
class AdsPackages_c extends BaseController{
    private $Login_m;
    private $security;     
    protected $session;
    public function __construct() {   
        ...
        $this->AdsPackages_m = new AdsPackages_m();
    ...
    public function view(){    
        $data['title'] = VIEW_ADS_PACKAGES; 
        echo admin_view('admin/view_ads_packages',$data);
    }
    ...
    public function add(){
        ...
        if (!$has_error && (is_array($package_details) && sizeof($package_details) > 0)) {
          ...
          $packageId = $this->AdsPackages_m->create_package($package_details);
    
          if ($packageId) {              
    
            successOrErrorMessage("Package has been successfully created", 'success');
            return redirect()->to(ADMIN_VIEW_ADS_PACKAGES_LINK);
          }
        ...
        $data['title'] = ADD_ADS_PACKAGES; 
        echo admin_view('admin/add_ads_packages',$data);
    }
    ...
    public function edit_ads_package($package_id){
        helper('form');  
        $package_details = $this->AdsPackages_m->get_package_details($package_id);
        ...
        if ($submitForm && !$has_error && (is_array($package_details) && sizeof($package_details) > 0)) {
          ...
          $update = $this->AdsPackages_m->update_package($package_details,$package_id);
        ...
        $data['package_details'] = $package_details;
        $data['title'] = ADMIN_EDIT_ADS_PACKAGES_TITLE; 
        echo admin_view('admin/add_ads_packages',$data);
      }
    ...
    public function update_status(){
        ...
        $update =  $this->AdsPackages_m->update_package_status($package_status,$package_id);
